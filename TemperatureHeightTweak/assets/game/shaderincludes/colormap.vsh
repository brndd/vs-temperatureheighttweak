...
void calcColorMapUvs(int colormapData, vec4 worldPos, float sunlightLevel, bool isLeaves) {
	...
	if (seasonMapIndex >= 0) {
		...
		if ((colormapData & 0xc0) == 0x40)
		{
			...
			//I don't even know what the shader uses this FOR,
			//but for it to remain accurate we need the inverse of our modded function which is a mouthful. --burneddi
			x += max(0, log(188.0 / (seaLevelDist + 1.78) - 1.0) / -0.03 + 155.0);
